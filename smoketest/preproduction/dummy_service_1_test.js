(function () {
  ...
  describe('Smoketested post-deployment for Dummy Service 1 end points', function () {
    ...
    it.only('will create sopa client from Dummy Service 1 wsdl', function (done){
      soap.createClient(config.dummyService1.wsdl, function (err, client) {
      ...
      it('returns a response when the getDummyService method is called', function (done) {
        soap.createClient(config.dummyService1.wsdl, function (err, client) {
          client.addSoapHeader(config.dummyService1.soapHeader);
            var soapBody = config.dummyService1.soapBody(parameters);
          ...
          client.getDummyService(soapBody, function (err, soapResponse) {
            ...
            expect(soapResponse.getDummyService1.MsgStatus).to.exist;
